...
Click or tap the purple `+` button to add a task. You can also press <kbd>CMD</kbd> + <kbd>N</kbd>.
...
To begin editing a task, simply double click it (or tap it on iOS). Or select it with the arrow keys and press <kbd>Return</kbd>. You can then type away.
The buttons bottom-right allow you to take quick action on the task: move it, schedule it, 
or many other actions via the &hellip; menu.
...
Right click a task and select `Edit Repetition` to make any task a repeating task. 
If you want to know more about repeating tasks and see all the features in action, check out this [video](/guides/repeating-tasks).

<Frame caption="Editing task repetition">
  <img src="/images/tasks/recurring.png" />
</Frame>
