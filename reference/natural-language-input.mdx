...
description: Type dates, times, projects, and tags straight into a task's name — Sonto files them for you
---
...
Type `!!!` anywhere in the name to mark the task as [priority](/reference/tasks). The marker turns into
a red pill as you type, and is lifted out of the name when you're done — `Call the bank !!!` becomes
**Call the bank**, marked priority.

| You type | Result |
| --- | --- |
| `Renew insurance !!!` | Named **Renew insurance**, marked priority |
| `!!! Call the bank` | Named **Call the bank**, marked priority |

It has to be exactly three marks: `!!` and `!!!!` are left alone as ordinary text, so an excitable task
name keeps its punctuation. Fullwidth `！！！` works too, for Chinese and Japanese keyboards.

<Tip>
  Delete the pill while you're still editing and the task goes back to whatever it was before — so this
  never overrides a priority you set with <kbd>Cmd</kbd> + <kbd>T</kbd>.
</Tip>
...
- **Markers don't overlap** — if `!!!` runs into a date, a duration or a `#`/`@` name, the longer match
  wins and the marker stays as text.
...
Dutch, German, French, Spanish, Portuguese, Japanese, and Chinese**; other languages fall back to
English. So `koop melk morgen` schedules "buy milk" for tomorrow on a Dutch system, just as
`buy milk tomorrow` does in English.

That list is one short of the [languages Sonto is translated into](/reference/languages): the app
speaks Korean, but typed dates aren't read in Korean yet. Projects, tags and `!!!` work everywhere,
since they match names you've made yourself.
