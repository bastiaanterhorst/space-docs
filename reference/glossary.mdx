...
A mark you put on a task so it stands out and surfaces on your day. Toggle it with
<kbd>Cmd</kbd> + <kbd>T</kbd>, from a task's `…` menu, or by typing `!!!` in the name. See
...
The atomic unit of work in Sonto. A task can be marked [priority](#priority), scheduled to a day or
week, placed on the calendar as a [time block](#time-block), filed in a project or area, tagged, and
given notes. See [Tasks](/reference/tasks).
